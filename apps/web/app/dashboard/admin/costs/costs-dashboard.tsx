  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="flex gap-1 bg-gray-800 rounded-lg p-1 light:bg-gray-100">
          {PERIODS.map((p) => (
            <button
              key={p.key}
              onClick={() => setPeriod(p.key)}
              className={`text-xs px-3 py-1.5 rounded-md transition-colors ${
                period === p.key ? "bg-gray-700 text-white light:bg-white light:text-gray-900" : "text-gray-400 hover:text-gray-200 light:text-gray-600 light:hover:text-gray-700"
              }`}
            >
              {p.label}
            </button>
        ...
        {loading && <span className="text-xs text-gray-400 animate-pulse light:text-gray-600">Carregando...</span>}
      </div>
      ...
      {data && (
        <>
          ...
          <section className="bg-gray-800 rounded-xl p-5 border border-orange-900/50 light:bg-gray-100">
            <div className="flex items-baseline justify-between flex-wrap gap-2">
              <h2 className="text-white font-semibold light:text-gray-900">
                A partir de 01/10/2026
                {data.projection.estimated && (
                  <span className="ml-2 text-xs font-normal text-orange-400 light:text-orange-700">estimativa</span>
                )}
              </h2>
              <span className="text-xs text-gray-400 light:text-gray-600">mesmo volume, regras novas</span>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4">
              ...
              <Card
                ...
                plain
                accent={data.projection.delta > 0 ? "text-orange-400 light:text-orange-700" : undefined}
              />
            </div>
            <p className="text-xs text-gray-400 mt-3 light:text-gray-600">
          ...
          <section className="bg-gray-800 rounded-xl p-5 light:bg-gray-100">
            <h2 className="text-white font-semibold mb-4 light:text-gray-900">Custo por dia</h2>
            {data.byDay.length === 0 ? (
              <p className="text-xs text-gray-400 py-6 text-center light:text-gray-600">Nenhum disparo registrado no período.</p>
            ) : (
              <div className="flex items-end gap-1" style={{ height: 120 }}>
                {data.byDay.map((d) => (
                  <div key={d.day} className="flex-1 flex flex-col items-center gap-1" title={`${d.day}: ${formatCurrency(d.cost)} · ${d.messages} msg`}>
                    ...
                    <span className="text-xs text-gray-400 light:text-gray-600">{d.day.slice(8)}</span>
                  </div>
                ))}
              </div>
            )}
          ...
          {data.byCampaign.length > 0 && (
            <section className="bg-gray-800 rounded-xl p-5 light:bg-gray-100">
              <h2 className="text-white font-semibold mb-4 light:text-gray-900">Custo por campanha</h2>
              <div className="space-y-3">
                {data.byCampaign.map((c) => (
                  <div key={c.campaignId} className="flex items-baseline justify-between gap-3">
                    <div className="min-w-0">
                      <p className="text-sm text-gray-300 truncate light:text-gray-700">{c.name}</p>
                    ...
                    <span className="text-sm text-white tabular-nums shrink-0 light:text-gray-900">{formatCurrency(c.cost)}</span>
                  </div>
                ))}
              </div>
            </section>
          ...
          <section className="bg-gray-800 rounded-xl p-5 light:bg-gray-100">
            <h2 className="text-white font-semibold mb-1 light:text-gray-900">Conferência com a Meta</h2>
            <p className="text-xs text-gray-400 mb-4 light:text-gray-600">
...
function Card({ label, value, sub, plain, accent }: {
  ...
  return (
    <div className={plain ? "" : "bg-gray-800 rounded-xl p-5 light:bg-gray-100"}>
      <p className="text-gray-400 text-sm light:text-gray-600">{label}</p>
      <p className={`text-2xl font-bold mt-1 tabular-nums ${accent ?? "text-white light:text-gray-900"}`}>{value}</p>
...
function Breakdown({ title, rows }: {
  ...
  return (
    <section className="bg-gray-800 rounded-xl p-5 light:bg-gray-100">
      <h2 className="text-white font-semibold mb-4 light:text-gray-900">{title}</h2>
      {rows.length === 0 ? (
        <p className="text-xs text-gray-400 py-4 text-center light:text-gray-600">Sem dados no período.</p>
      ) : (
        <div className="space-y-3">
          {rows.map((r) => (
            <div key={r.key}>
              <div className="flex justify-between text-sm mb-1 gap-3">
                <span className="text-gray-300 truncate light:text-gray-700">{r.label}</span>
                <span className="text-gray-400 tabular-nums shrink-0 light:text-gray-600">
                  {formatCurrency(r.cost)}
                  <span className="text-gray-400 light:text-gray-600"> · {r.messages.toLocaleString("pt-BR")}</span>
                </span>
              </div>
              <div className="w-full bg-gray-700 rounded-full h-1.5 light:bg-gray-200">
