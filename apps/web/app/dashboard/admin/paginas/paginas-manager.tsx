  ...
  return (
    <div className="space-y-6">
          ...
          <button
            ...
            className={`text-xs px-3 py-1.5 rounded-md border ${
              p.id === aberta
                ...
                : "bg-gray-900 border-gray-800 text-gray-400 hover:text-white light:bg-white light:border-gray-200 light:text-gray-600 light:hover:text-gray-900"
            }`}
          >
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-lg p-4 space-y-3 light:bg-white light:border-gray-200">
        <p className="text-xs font-medium text-white light:text-gray-900">Nova página</p>
      ...
      {pagina && (
        <>
          {/* ─── Identidade ─── */}
          <section className="bg-gray-900 border border-gray-800 rounded-lg p-4 space-y-3 light:bg-white light:border-gray-200">
            <div className="flex items-center justify-between">
              <p className="text-xs font-medium text-white light:text-gray-900">Perfil</p>
            ...
            <label className="block text-xs text-gray-400 space-y-1 light:text-gray-600">
            ...
            <div className="flex items-center gap-3">
              ...
              <label className="text-xs text-gray-400 light:text-gray-600">
                <span className="block mb-1">
                  ...
                  {pagina.avatar_path && (
                    <button
                      ...
                      className="ml-2 text-gray-400 hover:text-red-400 light:text-gray-600"
                    >
                      remover
                    </button>
                  )}
          ...
          {/* ─── Tema ─── */}
          <section className="bg-gray-900 border border-gray-800 rounded-lg p-4 space-y-3 light:bg-white light:border-gray-200">
            <p className="text-xs font-medium text-white light:text-gray-900">Aparência desta página</p>
            ...
            <label className="block text-xs text-gray-400 space-y-1 max-w-[220px] light:text-gray-600">
          ...
          <section className="space-y-3">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <p className="text-xs font-medium text-white light:text-gray-900">Blocos ({daPagina.length})</p>
              <div className="flex gap-2">
                {(["agenda", "link", "texto", "imagem"] as const).map((t) => (
                  <button
                    key={t}
                    onClick={() => novoBloco(t)}
                    className="text-xs px-2 py-1 rounded-md bg-gray-800 border border-gray-700 text-gray-300 hover:text-white light:bg-gray-100 light:border-gray-300 light:text-gray-700 light:hover:text-gray-900"
                  >
                    + {ROTULO_TIPO[t]}
                  </button>
                ))}
          ...
          {isAdmin && (
            <button
              ...
              className="text-xs text-gray-400 hover:text-red-400 light:text-gray-600"
            >
              remover esta página
            </button>
          )}
...
function BlocoEditor({
  ...
  return (
    <div className={`bg-gray-900 border rounded-lg p-4 space-y-3 light:bg-white ${bloco.ativo ? "border-gray-800 light:border-gray-200" : "border-gray-800/50 opacity-60 light:border-gray-200/60"}`}>
      <div className="flex items-center justify-between gap-3">
        <p className="text-xs text-gray-300 light:text-gray-700">
          {ROTULO_TIPO[bloco.tipo]}
          {bloco.cliques > 0 && <span className="text-gray-400 light:text-gray-600"> · {bloco.cliques} clique(s)</span>}
        </p>
        <div className="flex items-center gap-2 text-xs">
          <button onClick={() => onMover(-1)} disabled={primeiro} className="text-gray-400 hover:text-white disabled:opacity-30 light:text-gray-600 light:hover:text-gray-900">↑</button>
          <button onClick={() => onMover(1)} disabled={ultimo} className="text-gray-400 hover:text-white disabled:opacity-30 light:text-gray-600 light:hover:text-gray-900">↓</button>
          <button onClick={() => onSalvar({ ativo: !bloco.ativo })} className="text-gray-400 hover:text-white light:text-gray-600 light:hover:text-gray-900">
            {bloco.ativo ? "esconder" : "mostrar"}
          </button>
          <button onClick={onRemover} className="text-gray-400 hover:text-red-400 light:text-gray-600">remover</button>
        </div>
      </div>
      ...
      {bloco.tipo === "agenda" && (
        <>
          ...
          <p className={`text-xs ${quantos > 0 ? "text-gray-400 light:text-gray-600" : "text-amber-400 light:text-amber-700"}`}>
            {artistaDaPagina
              ? quantos > 0
...
function ImagemDoBloco({
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="text-xs text-gray-400 space-y-1 light:text-gray-600">
        ...
        <input
          ...
          className="block text-xs text-gray-300 light:text-gray-700"
        />
      </div>
    </div>
  );
    ...
    <label className="block text-xs text-gray-400 space-y-1 light:text-gray-600">
      ...
      <div className="flex items-center gap-2">
        <input
          ...
          className="h-9 w-12 rounded bg-gray-800 border border-gray-700 light:bg-gray-100 light:border-gray-300"
        />
        <span className="font-mono text-gray-400 light:text-gray-600">{valor}</span>
      </div>
    </label>
  );
}
