...
export default function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center text-center py-12 px-6">
      {Icon && (
        <div className="mb-3 flex h-10 w-10 items-center justify-center rounded-full bg-gray-800 text-gray-400 light:bg-gray-100 light:text-gray-600">
          <Icon size={18} aria-hidden="true" />
        </div>
      )}
      <p className="text-sm font-medium text-gray-300 light:text-gray-700">{title}</p>
      {description && <p className="mt-1 max-w-sm text-xs text-gray-400 light:text-gray-600">{description}</p>}
